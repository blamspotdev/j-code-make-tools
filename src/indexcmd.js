...
const { normalizeHeader } = require('./pack');
...
function index(marketplaceDir, opts = {}) {
  ...
  for (const file of jexts) {
    ...
    if (buf.length > 9 && buf.subarray(0, 4).toString('ascii') === 'JEXT' && buf[4] === 2) {
      ...
      // Plain zip (format 1): the header is extension.yaml, normalized the same way `pack` does.
      const zip = new AdmZip(buf);
      const yEntry = zip.getEntry('extension.yaml');
      if (!yEntry) { warn(`${file}: no extension.yaml — skipped`); continue; }
      header = normalizeHeader(yaml.load(zip.readAsText(yEntry)) || {});
  ...
  const header =
    ...
    '# edit each extension\'s extension.yaml and re-pack, do not hand-edit entries.\n' +
